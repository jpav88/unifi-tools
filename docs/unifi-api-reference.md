...
curl -sk -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  "$BASE/stat/sta/xx:xx:xx:xx:xx:xx"
```
...
CLIENT_ID=$(curl -sk -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  "$BASE/stat/alluser" | jq -r '.data[] | select(.mac == "xx:xx:xx:xx:xx:xx") | ._id')
...
curl -sk -X POST -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  -H "Content-Type: application/json" \
  -d '{"cmd":"kick-sta","mac":"xx:xx:xx:xx:xx:xx"}' \
  "$BASE/cmd/stamgr"

# Block a client
-d '{"cmd":"block-sta","mac":"xx:xx:xx:xx:xx:xx"}'

# Unblock
-d '{"cmd":"unblock-sta","mac":"xx:xx:xx:xx:xx:xx"}'

# Forget a client (remove from known devices — takes up to 5 minutes)
-d '{"cmd":"forget-sta","macs":["xx:xx:xx:xx:xx:xx"]}'
```
...
curl -sk -X POST -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  -H "Content-Type: application/json" \
  -d '{"cmd":"restart","mac":"xx:xx:xx:xx:xx:xx"}' \
  "$BASE/cmd/devmgr"

# Force provision (re-push config to device)
-d '{"cmd":"force-provision","mac":"xx:xx:xx:xx:xx:xx"}'

# Flash LED to locate a device
-d '{"cmd":"set-locate","mac":"xx:xx:xx:xx:xx:xx"}'
# Stop flashing
-d '{"cmd":"unset-locate","mac":"xx:xx:xx:xx:xx:xx"}'

# Adopt a new device
-d '{"cmd":"adopt","macs":["xx:xx:xx:xx:xx:xx"]}'
```
...
curl -sk -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  "$BASE/stat/device" | jq '.data[] | select(.mac == "xx:xx:xx:xx:xx:xx") | {
    name,
...
```json
{
  ...
  "parameters": {
    "DEVICE": {"name": "U7 Pro Max", "id": "xx:xx:xx:xx:xx:xx", "model": "U7PROMAX"},
...
curl -sk -X POST -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  -H "Content-Type: application/json" \
  -d "{\"type\":\"all\",\"start\":$START,\"end\":$END,\"mac\":\"xx:xx:xx:xx:xx:xx\"}" \
  "$BASE/stat/session"

# Latest 5 sessions for a client (sorted by most recent)
curl -sk -X POST -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  -H "Content-Type: application/json" \
  -d '{"mac":"xx:xx:xx:xx:xx:xx","type":"all","_limit":5,"_sort":"-assoc_time"}' \
  "$BASE/stat/session"
```
...
curl -sk -b "$COOKIES" -H "X-Csrf-Token: $CSRF" \
  "$BASE/stat/spectrum-scan/xx:xx:xx:xx:xx:xx"
```
