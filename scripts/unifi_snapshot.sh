...
# EXPECTED_SLOW_PORTS (local/devices.sh) lists "<switch> P<idx>" entries to skip —
# e.g. a device whose NIC tops out at 100M
EXPECTED_SLOW_JSON=$(printf '%s\n' "${EXPECTED_SLOW_PORTS[@]:-}" | jq -R . | jq -s 'map(select(length > 0))')
SW_ALERTS=$(echo "$SWITCHES" | jq -r --argjson expected "$EXPECTED_SLOW_JSON" '
    [.[] | .name as $sw | .ports[] |
        select(.up == true and .speed < 1000) |
        "\($sw) P\(.port_idx)" as $label |
        select(($expected | index($label)) == null) |
        "\($label):\(.speed)M"
